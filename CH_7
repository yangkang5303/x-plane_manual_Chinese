...
初学者难懂的原因：其中可能的原因是要使用它们必须先设置好正确的频率和HSI模式，还有可能是因为直到他们找到想要的进近路线，才会触发工作。
	自动飞行必须先被告知是ILS VOR 还是GPS模式，才会用其中一个模式去飞行。
自动飞行会按照HSI（如果有的话）的指向飞行，通过NAV GPS按钮来告诉HSI的方位指向。
	如果你按下NAV1，那么HSI就会指向NAV1无线电指向的航标，当你按下LOC（和G/S），飞机就话自动飞VOR或者ILS。
NAV2同理，不过有的飞机NAV2不能飞ILS盲降。
	如果你把开关指向FMC/CDU，那么HSI就会指向GPS的目标方向。按下LOC自动飞向那个方向。在FMS里面也可以单独输入并且触发。
	重复：当使用LOC和G/S的时候一定要确保向自动飞行发送了正确的信号。
	LOC被按下之后，自动飞行系统会立即开始径向导航（GPS），但是，飞VOR或者ILS的时候，如果离导航台太远或者角度不对，LOC指示就是黄色的（待命状态），直到导航仪表上的针突然回中的时候，LOC才会进入工作（切断之前所有的模式）。
	会切断之前所有模式的原因是：你可能在飞HDG模式，一旦导航针“进来”，你希望自动飞行忘记这个指向，飞向跑道的方向，或者其他某个特定方向。有趣的是，垂直飞行喝着个有点类似——在你没有到达莫个高度时你可以做任何动作，一旦飞到这个高度，飞机就会进入高度保持模式。
	和径向导航一样，垂直导航在没有看见下滑针移动之前是不会做任何事情的，有点不同的是：直到下滑指示针碰到中心线的时候，才会触发下滑动作。这个可以想象：飞机在切入下滑道之前正保持着高度向前飞行。一旦切入，飞机立刻放弃个哦度保持，向下降。换句话说，G/S模式只有触碰到G/S的中央时才会工作！
...
下面以波音777为例说明FMS用法。
	...
	2.FMS在屏幕的下面，靠近面板的中间，按下FMS上的INIT键。
	3.按下AIRP键，告诉FMS你即将去往某一个机场。
	4.现在用鼠标输入一个目标机场的代号，例如从圣地亚哥国际机场：KSAN，飞往圣贝纳迪诺国际机场（KSBD）
	5.如果你喜欢，按下FMS左边的航线选择来飞特定航路（在“FLY AT——FT”旁边），然后输入你想飞的高度。
	6.现在，如果你要做的不仅仅是飞向一个机场，按下NEXT键，重复上述步骤。
		用回退键来更正错误，用VOR，NDB，FIX和LAT/LON键输入这些类型的目的地。按PREV和NEXT键往前往后翻页，按LD和SA在加载和保存飞行计划。
	7.一旦输入FMS计划，起飞，并且把SOUCE键指向GPS（面板的左边角）。
	8.把FLIGHT DIR指向AUTO，让飞机自己接管飞行，如果你输入了高度，按下VNAV键让飞机自己飞到那个高度。
	9.找空姐聊聊天什么的去吧，飞机将自己飞往目的地。
...
开始讨论仪表飞行之前，我们必须讨论仪表飞行为什么这么困难，困难在于我们必须相信仪表告诉我们的数据，我们自身的平衡感在过去几百万年的进化阶段中，已经十分成熟，强迫自己忘掉这些身体的感觉，去相信仪表给我们的数据十分困难。
作者讲了一个坐在旋转椅子上被人告诉旋转的事例，人的内部平衡传感器和人眼看到的景象不相符合的时候，就会很晕。
在驾驶舱里面，同样的事情也会上演，假象你驾驶一些乘客以左斜20度的角度进入一大片云朵，在慢慢进入云朵之后，你开始把飞机往右倾斜，如果你做得尽量慢没有人会意识到这一点，在出云层之前，有的乘客可能就怀疑你是否向左倾斜的！当你飞出云层的尽头的时候，乘客们会大呼：“怎么往右转了！”这个玩笑是无害的，不过有些粗心飞行员会把自己诓进去。
...
2.转向指示
		3.方向指示
高度指示（AI）表明了飞机现在所在的高度——机头距离地面的距离，转向指示（TC）表明了你的转向度和标准2分钟转弯度相比，DG就是一个精度比传统磁性指南针高的“指南针”。

主飞行仪表
		
6个主飞行仪表在上世纪70年代被定为飞机上的主仪器标准，分为上下两行，每行各3个，上面一行，从左往右，分别是空速表，飞行姿态，高度表；下面一行分别是：转向指示，方向指示，垂直速度表。
	空速表顾名思义，表示的是飞机相对于空气的速度，速度传感器放在机翼下面或者机头下面，根据风吹在上面形成的压力来判定当前的空速。
	姿态表显示了飞机相对水平线的位置，这个是通过修正飞机的陀螺仪的感知数据算出来的。
	高度表看上去有点像种面，高度表的数据来源是通过一系列分布式的或者集中的传感器搜集空气的表象获得的：当飞机上升或者下降，机外的气压发生变化，然后通过一些相对值的对比，算出此时的飞行高度。
	转向指示，表达的是飞机的转向率，如果飞机像汽车一样“漂移”的话，这个表是不准的。
	方向指示告诉飞行员飞机现在的飞行方向。
	垂直速度表，显示的是飞机每分钟正在爬升或者下潜的速度，一般情况下，飞机每分钟爬升700英尺，下潜500英尺/分钟是让人感觉舒服的，如果下潜过快，乘客的耳朵里面的传感器就会感觉出来，恒压机舱的飞机可以上升下降的速率快一些，因为机舱的压力和环境的气压是不一样的，如果一样的话，那就是气压装置坏了！
